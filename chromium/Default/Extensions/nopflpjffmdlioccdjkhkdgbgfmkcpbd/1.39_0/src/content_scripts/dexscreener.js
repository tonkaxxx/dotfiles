chrome.storage.local.get("tradewiz.config").then((config) => {
  ...
  window.platformMarketFloatingQuickBuyLog = "dexscreener-market-floating-quickbuy"
  window.platformMarketFloatingBuyLog = "dexscreener-market-floating-buy"
  window.platformPositionSellLog = "dexscreener-position-sell"
  ...
  chrome.runtime.onMessage.addListener(async (request) => {
    if (request.message === "dexscreener-token") {
      ...
      removeQuickPanel();
      createQuickPanel();
    } else if (request.message === "dexscreener-token-bsc") {
      await setChain('BSC');
      removeQuickPanel();
