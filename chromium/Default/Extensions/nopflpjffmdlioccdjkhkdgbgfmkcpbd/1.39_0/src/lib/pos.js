 ...
 * @returns 
 */
const getTokenPositionInfo = (tokenInfo, chain, bnbPrice) => {
  if (chain == "SOL") {
    const { token, tradingSummary } = tokenInfo
    if (!token) {
      return {
        tokenBalanceInSol: 0,
        tokenBalanceInUsd: 0,
    ...
    const priceInBNB = parseFloat(tokenInfo.priceInBnb || 0) * 10 ** tokenInfo.decimals / 10 ** 18;
    const tokenBalance = tokenInfo.tokenBalance / 10 ** tokenInfo.decimals;
    const tokenBalanceInBNB = (
      tokenBalance ? tokenBalance * priceInBNB : 0
    )
    const tokenBalanceInUsd = tokenBalance * priceInBNB * bnbPrice
    const profitInUsd = tokenBalanceInUsd + tokenInfo.sellAmountInUsd - tokenInfo.buyAmountInUsd
    const profitInBNB = profitInUsd / bnbPrice
    const profitRate = tokenInfo.buyAmountInUsd ? (profitInUsd / tokenInfo.buyAmountInUsd) * 100 : 0
    return {
      tokenBalanceInBNB,
      tokenBalanceInUsd,
      profitInBNB,
 ...
 * @param {*} param1 
 */
async function createPositionList(panel, { limitOrders }) {
