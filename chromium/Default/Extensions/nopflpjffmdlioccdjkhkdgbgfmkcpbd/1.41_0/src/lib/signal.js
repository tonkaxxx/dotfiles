...
const setPlatformSignalStyle = async (platform, container) => {
  if (platform === 0) {
    ...
    window.platformLayout = await findContainer(".min-w-\\[1024px\\].flex-1.overflow-auto");
    if (!platformLayout) return;
    if (container) {
      container.style.width = "fit-content";
      window.platformLayout.style.display = "flex"
      platformLayout.insertBefore(container, platformLayout.firstChild);
    }
  } else if (platform === 13) {
    window.platformLayout = await findContainer(".home-container");
    if (!platformLayout) return;
    if (container) {
      container.style.width = "fit-content";
      const root = await findContainer("#root");
      root.style.width = "0"
      root.style.flex = "1"
      window.platformLayout.style.display = "flex"
      platformLayout.insertBefore(container, platformLayout.firstChild);
