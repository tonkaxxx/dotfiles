...
async function setPanelDataBSC(panel, data, type) {
    ...
    try {
        if (type === 'trade') {
            ...
            const posTokenCard = document.querySelector(`[data-token="${data.token_address?.toLowerCase()}"].tradewiz-token-card`);
            const currentTokenInfo = panel.querySelector(`[data-token="${data.token_address?.toLowerCase()}"].tradewiz-current-token-info`);
            if (posTokenCard) {
                if (tokenBalance > 0) {
            ...
            if (currentTokenInfo) {
                ...
                //     posTokenPnl.innerText = `${formatProfitAndLoss(totalProfit)}(${formatProfitRate(profitRate)})`;
                // }
                const tokenBalance = Number(currentTokenInfo.getAttribute("token-balance")) || 0;
                const tokenBalanceInUsd = tokenBalance * Number(data.price_usd);
                const buyAmountInUsd = Number(currentTokenInfo.getAttribute("buy-amount-in-usd")) || 0;
                const sellAmountInUsd = Number(currentTokenInfo.getAttribute("sell-amount-in-usd")) || 0;
                ...
                const tokenBalanceEl = currentTokenInfo.querySelector(".tradewiz-current-token-balance");
                const tokenPnlElement = currentTokenInfo.querySelector(".tradewiz-current-token-pnl");
                if (tokenBalanceEl) {
                    tokenBalanceEl.innerText = `$${tokenBalanceInUsd.toFixed(2)}`;
                }
                if (tokenPnlElement) {
                    if (Number(profit) > 0) {
                        tokenPnlElement.style.color = "#7FFA8B"
                    } else if (Number(profit) < 0) {
                        tokenPnlElement.style.color = "#FF4D67"
                    } else {
                        tokenPnlElement.style.color = "#fff"
                    }
                    tokenPnlElement.innerText = `${formatProfitAndLoss(profit)}(${formatProfitRate(profitRate)})`;
                }
            }
        }
    } catch (error) {
