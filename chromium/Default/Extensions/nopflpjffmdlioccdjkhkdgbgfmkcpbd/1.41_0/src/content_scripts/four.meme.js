chrome.storage.local.get("tradewiz.config").then((config) => {
    ...
    async function addPulseButtons(card) {
        ...
        const quickBtn = card.querySelector("button.flex-h-cc.gap-1");
        if (quickBtn) {
    ...
    async function findFourMemeTokenContainer(timeout = 10000) {
        await new Promise((r) => setTimeout(r, 500));
        for (let i = 0; i < timeout / 500; i++) {
            const tables = Array.from(document.querySelectorAll('div.overflow-auto.advanced-scrollbar'))
            if (tables.length) {
                return tables;
            }
            await new Promise((r) => setTimeout(r, 500));
    ...
    const disconnectObservers = () => {
        const discoverObservers = observers["four-meme"] || [];
        discoverObservers.forEach((observer) => observer.disconnect());
    }
    async function watchFourMeme() {
        const tables = await findFourMemeTokenContainer();
        if (!tables || tables.length === 0) return;
        if (tables && tables.length > 0) {
            for (const table of tables) {
                setTimeout(() => {
                    const rows = Array.from(table.querySelectorAll('div.flex-v.border-b.cursor-pointer.border-darkGray800'));
                ...
                observers["four-meme"] = observers["four-meme"] || [];
                observers["four-meme"].push(observer);
            }
        }
    }

    chrome.runtime.onMessage.addListener(async (request) => {
        if (request.message === "four-meme-token") {
            await setChain('BSC');
            removeQuickPanel();
            createQuickPanel(undefined, 'BSC');
        } else if (request.message === "four-meme") {
            // await setChain('BSC');
            // initialConnection();
            // disconnectObservers();
            // watchFourMeme();
