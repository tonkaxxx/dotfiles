chrome.storage.local.get("tradewiz.config").then((config) => {
  ...
  window.platformMarketFloatingQuickBuyLog = "xxyy-market-floating-quickbuy"
  window.platformMarketFloatingBuyLog = "xxyy-market-floating-buy"
  window.platformPositionSellLog = "xxyy-position-sell"
  window.keys = {
  ...
  chrome.runtime.onMessage.addListener(async (request) => {
    if (request.message === "xxyy-token") {
      await setChain('SOL');
      removeQuickPanel();
      createQuickPanel(undefined, "SOL");
    } else if (request.message === "xxyy-token-bsc") {
      await setChain('BSC');
      removeQuickPanel();
