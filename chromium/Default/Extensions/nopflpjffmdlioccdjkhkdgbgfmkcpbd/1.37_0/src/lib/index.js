...
async function createQuickPanelButton(panel, amounts, text, preset, callback = async () => { }, args) {
    ...
    buyPanelTitle.innerHTML = `
      <div>${text}</div>
        <div style="display: flex; align-items: center; gap: 4px;font-size: 12px;color: #ADADCC;">
          <img src="${chrome.runtime.getURL("src/public/assets/images/solana1.png")}" width="14" height="14" />
          <span class="tradewiz-sol-balance">${(balance / 10 ** 9).toFixed(3)}</span>
        </div>
      </div>
    `
  } else if (text === "Sell") {
    const { tokenBalanceInUsd = 0, symbol = "", tokenBalance = 0, profitInUsd = 0, profitRate } = getTokenPositionInfo(args?.current || {});
    buyPanelTitle.innerHTML = `
  ...
  buyContainer.innerHTML = `
    ...
    <div style="display: flex; align-items: center; gap: 6px;" data-tip="${text} Fee">
    ...
    <div style="width:1px;height:14px;background:rgba(173, 173, 204, 0.1);margin:0 4px;"></div>
    <div style="display: flex; align-items: center; gap: 6px;" data-tip="${text} Tip">
  ...
  if (text === "Buy" && window.platform === 0) {
   ...
   wholeCheckbox.querySelector('input').addEventListener("change",(e)=>{
    anyCheckbox.querySelector('input').checked = !e.target.checked
   })
   anyCheckbox.querySelector('input').addEventListener("change",(e)=>{
...
const createEditPresetPanel = async (isActive = false) => {
 ...
 * create quick panel
 */
async function createQuickPanel(address = "") {
  if (isCreatingPanel) {
    return;
  }
  ...
  const editPreset = createButtonIcon({
    ...
    onClick: (e, _, isActive) => {
      createEditPresetPanel(isActive);
    }
  ...
  if (Array.isArray(presetValue) && presetValue.length) {
    ...
    presetSelect.addEventListener("change", (e) => {
      ...
      if (JSON.stringify(newPresetValue) !== JSON.stringify(presetValue)) {
        chrome.storage.local.set({ "tradewiz.newPreset": newPresetValue }, () => {
          removeQuickPanel();
          createQuickPanel();
        });
      }
  ...
  let response = {
    balance: await getStoredValue("tradewiz.balance") || 0,
    current: {},
    tokens: [],
    solPrice: await getStoredValue("tradewiz.solPrice") || 0
  }
  await createQuickPanelButton(
    ...
    {
      ...response,
      publicKey: address || extractToken()
    });
  await createQuickPanelButton(buySellContainer, (selectedPreset?.values?.sellPercents || [10, 20, 50, 100]).concat("initial"), "Sell", selectedPreset, (arg) => tradeCallback({
    ...arg,
    selectedPreset,
    tokenAddress: address
  }), {
...
async function initPositionInfo(panel, address, update = false) {
  try {
    ...
    if (tokenResponse?.tradingInfo && tokenInfoElement) {
      const { tokenBalanceInUsd = 0, symbol = "", tokenBalance = 0, profitInUsd = 0, profitRate } = getTokenPositionInfo(tokenResponse.tradingInfo);
      if (tokenBalance > 0) {
...
async function tradeLimitCallback({
  ...
  } = await chrome.storage.local.get(["tradewiz.antiMev"]);
